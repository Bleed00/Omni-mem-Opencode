"""Pretty terminal UI for Omni-mem.

Wraps ``rich``/``questionary`` with a plain-text fallback, so every helper
works even when the dependencies are not installed (the installer bootstraps
them via ``pip install -e .`` when they are missing).
"""
